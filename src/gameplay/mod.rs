...
pub fn update(gamepad: &mut gilrs::Gilrs, local_player: &mut player::LocalPlayer, render_data: &mut render::RenderData, orient: &UnitQuaternion<f32>,
    ...
    let matrix = UnitQuaternion::from_quaternion(Quaternion::new(local_player.camera_rotation.0, local_player.camera_rotation.1,local_player.camera_rotation.2,local_player.camera_rotation.3)).to_homogeneous();
    ...
    if (local_player.player_speed_f == 0.0) & (local_player.player_speed_lr == 0.0){
        ...
        local_player.rotation = local_player.camera_rotation;
    }
    else{
    ...
    if local_player.player_moving {
        ...
        let ghost_rot = UnitQuaternion::from_quaternion( Quaternion::new(ghost_rot.0, ghost_rot.1, ghost_rot.2, ghost_rot.3) ) * orient;
        let ghost_rot = (ghost_rot[0], ghost_rot[1], ghost_rot[2], ghost_rot[3]);
